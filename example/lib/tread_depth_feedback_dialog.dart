...
class TreadDepthResultFeedbackDialog extends StatefulWidget {
  ...
  final List<TreadResultRegion> regions;
...
class _TreadDepthResultFeedbackDialogState
  ...
  bool isValueInMm = false;

  @override
  void initState() {
    isValueInMm = (widget.regions.first.valueMm != 0.0);

    for (int i = 0; i < widget.regions.length; i++) {
      _textController.add(TextEditingController(
          text: (widget.regions[i].valueMm != 0.0)
              ? '${widget.regions[i].valueMm ?? 0.0}'
              : '${widget.regions[i].valueInch ?? 0.0}'));
      views.add(SizedBox(
        width: 80,
        child: Column(
          ...
          children: [
            ...
            Text(
              (widget.regions[i].valueMm != 0.0) ? 'Mm' : 'Inch',
                  ...
                  List<TreadResultRegion> regions = _textController.map((data) {
                    if (isValueInMm) {
                      ...
                      return TreadResultRegion.initInch(
                          available: data.text.isNotEmpty,
                          valueInch: double.parse(data.text));
                    }
                  }).toList();
