...
class TireTreadPlugin {
  Future<String?> get sdkVersion => AnylineTireTreadPluginPlatform.instance.getSdkVersion();

  Future<String> get pluginVersion async {
    final fileContent = await rootBundle.loadString('packages/anyline_tire_tread_plugin/pubspec.yaml');
    final pubspec = Pubspec.parse(fileContent);
    return pubspec.version?.toString() ?? '';
  }

  Future<bool?> initialize(String licenseKey) {
    return AnylineTireTreadPluginPlatform.instance.initialize(licenseKey: licenseKey);
  }
  ...
  Future<String?> getResult({required String measurementUUID}) {
    return AnylineTireTreadPluginPlatform.instance.getResult(measurementUUID: measurementUUID);
  }

  Future<String?> getHeatMap({required String measurementUUID}) {
    return AnylineTireTreadPluginPlatform.instance.getHeatMap(measurementUUID: measurementUUID);
  }
